...
Python-библиотека, CLI `yd` / `yadisk` и приложение **«Яндекс Бэкап»** в строке меню macOS. Версия 0.3.2. Неофициальный клиент, не связан с Яндексом.
...
python macos/install.py \
  --wheel dist/yadisk_client_local-0.3.2-py3-none-any.whl \
...
Файлы сравниваются по содержимому. При каждом проходе читаются локальные файлы; загружаются только отличающиеся. Первая копия множества маленьких файлов может быть медленной из-за запросов API. Общая ограниченная очередь загружает файлы из разных папок параллельно. Чтение облачных каталогов совмещается с передачами. `.git` сохраняется пофайлово; упаковка в архивы не реализована. Повторная проверка staging после уже проверенной загрузки исключена; проверки содержимого upload и конечного файла сохранены.
...
85 тестов без доступа к настоящему Диску. Интеграционные проверки загрузки, версий и удаления локальных файлов выполнялись отдельно на временных данных. Длительная работа и сборка macOS Intel не проверены.
